...
export function checkMaker(
  ...
  // Sadece ilk 60 saniyede
  if (elapsed > 60 || elapsed < 0) return;
  ...
  // Her iki taraf da 49¢ civarında mı? (ask <= 0.51)
  if (!upAsk || !downAsk) return;
  if (upAsk > 0.52 || downAsk > 0.52) return;

  // Paper trade simülasyonu:
  // Gerçekte her iki tarafın dolması gerekiyor.
  // Simülasyonda: ask <= 0.51 ise "fill oldu" sayıyoruz
  const upFilled   = upAsk   <= BID_PRICE + 0.02;
  const downFilled = downAsk <= BID_PRICE + 0.02;

...
function topDepth(levels: OrderLevel[], n = 5): number {
  return levels.slice(-n).reduce((s, l) => s + Number(l.price) * Number(l.size), 0);
}

function midpoint(bids: OrderLevel[], asks: OrderLevel[]): number | null {
  const bestBid = bids.at(-1);
...
export async function takeSnapshot(
  ...
  const upBid  = upBook?.bids.at(-1)  ? Number(upBook.bids.at(-1)!.price)  : null;
  const upAsk  = upBook?.asks.at(-1)  ? Number(upBook.asks.at(-1)!.price)  : null;
  const dnBid  = downBook?.bids.at(-1) ? Number(downBook.bids.at(-1)!.price) : null;
  const dnAsk  = downBook?.asks.at(-1) ? Number(downBook.asks.at(-1)!.price) : null;
